...
l_graphs$graph6 <- readRDS("data/hoekstra_synth_9node.RDS")
...
for(t in seq_along(n_tp)){
  ...
  l_raw$graph5[[t]] <- sim_raw_parallel(dgp = l_graphs$graph5,
                                    ...
                                    standardize = TRUE)
  l_raw$graph6[[t]] <- list()
  l_raw$graph6[[t]] <- sim_raw_parallel(dgp = l_graphs$graph6,
                                    n = n_postds,
                                    tp = n_tp[t],
                                    seed = seed,
                                    means = 0,
...
# save evaluations
# saveRDS(df_eval_gvar, here::here("output/df_eval_gvar.RDS"))
...
df_eval_bggm_long <- df_eval_bggm %>% 
  # mutate tp_ind into factor - TODO does not work yet
  mutate(tp_ind = fct_recode(as_factor(as.character(tp_ind)), !!!tp_levels)) %>% 
  group_by(dgp_ind, tp_ind, rho_prior, beta_prior) %>% 
  # convert coverage into proportion
  mutate(n_nodes = ifelse(dgp_ind == 6, 8, 6)) %>% 
...
df_eval_bggm %>% 
  left_join(cond_grid, by = c("dgp_ind", "tp_ind", "rho_prior", "beta_prior"))
...
plot_eval_both <- df_eval_both %>% 
  ...
  dplyr::summarize(across(c(contains("bias"),contains("rmse")),
                   ~mean(., na.rm = TRUE))) %>% 
  ...
  mutate(method = case_when(
    stringr::str_detect(stat, "bggm_b0.2") ~ "bggm_narrow",
    stringr::str_detect(stat, "bggm_b1") ~ "bggm_wide",
    stringr::str_detect(stat, "gvar") ~ "gvar",
    stringr::str_detect(stat, "sel_b0.2") ~ "sel_narrow",
    stringr::str_detect(stat, "sel_b1") ~ "sel_wide"
  )) %>% 
  ...
  # only show bias for now
  ggplot(aes(x = timepoints, y = value, col = method, ))+
  geom_line()+
  # geom_point()+
  # facet_grid(as.factor(stat) + as.factor(mat) ~ as.factor(dgp_ind))+
  ggh4x::facet_nested(as.factor(stat) + as.factor(mat) ~ as.factor(dgp_ind),
                      scales = "free_y")+
  scale_x_continuous(breaks = c(50,100,200,400,1000))+
  theme_bw()+
  ggokabeito::scale_color_okabe_ito()+
...
ggsave("plot_eval_both_crazy.svg", plot_eval_both, height = 7, width = 10, path = here::here("figures/"), device = "svg")
