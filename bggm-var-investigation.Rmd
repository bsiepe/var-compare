...
Function to calculate rmse across all repetitions
```{r}
rmse_test <- eval_across_gvar(fit = tes,
            true_graph = l_graphs[[6]]) 


cool <- unlist(lapply(tes, function(x) as.numeric(c(t(x$beta[,-1])))))
...
plot_eval_both_select <- df_eval_both %>% 
  ...
  labs(x = "Timepoints",
       ...
       linetype = "Selection")+
  theme_compare()+
  theme(strip.text = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.2), hjust = 0.5),
        strip.text.x.top = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.1), hjust = 0.5),
        legend.text = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.1)),
        legend.title = ggplot2::element_text(face = "bold", size = ggplot2::rel(1.1)),
...
ggsave("plot_eval_both_select.svg", plot_eval_both_select, height = 6.5, width = 12, path = here::here("figures/"), device = "svg")
```
