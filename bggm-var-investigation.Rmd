...
saveRDS(df_eval_bggm, here::here("output/df_eval_bggm.RDS"))
...
plot_eval_both <- df_eval_both %>% 
  dplyr::select(c(dgp_ind, tp_ind, lambda, ebic, contains("bias"),contains("rmse"))) %>% 
  dplyr::group_by(dgp_ind, tp_ind, lambda, ebic) %>% 
  dplyr::summarize(across(c(contains("bias"),contains("rmse")),
                   ~mean(., na.rm = TRUE))) %>% 
  pivot_longer(cols = c(contains("bias"), contains("rmse")),
                        names_to = "stat") %>%
  mutate(method = case_when(
    stringr::str_detect(stat, "bggm") ~ "bggm",
    stringr::str_detect(stat, "gvar") ~ "gvar",
    stringr::str_detect(stat, "sel") ~ "sel",
  )) %>% 
  mutate(mat = case_when(
    stringr::str_detect(stat, "beta") ~ "beta",
    stringr::str_detect(stat, "pcor") ~ "pcor"
  )) %>% 
  ...
  theme_bw()+
  ggokabeito::scale_color_okabe_ito()+
  labs(caption = "GVAR default, BGGM rho = 0.5, beta = 1")
ggsave("plot_eval_both2.svg", plot_eval_both, height = 7, width = 10, path = here::here("figures/"), device = "svg")
