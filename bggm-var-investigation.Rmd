...
# BGGM
pcor_list <- lapply(fit_bggm[[131]], function(x)
  return(x$pcor_mu))
...
beta_list <- lapply(fit_bggm[[131]], function(x)
  return(x$beta_mu))

bb <- apply(simplify2array(beta_list), 1:2, mean)


# gVAR


PCC_gvar_list <- lapply(fit_gvar[[26]], function(x)
  return(x$PCC))

apply(simplify2array(PCC_gvar_list), 1:2, mean)

beta_gvar_list <- lapply(fit_gvar[[26]], function(x)
  return(t(x$beta[,-1])))

bg <- apply(simplify2array(beta_gvar_list), 1:2, mean)

bb-bg

l_graphs$graph1$beta-bb
l_graphs$graph2$beta-bb
l_graphs$graph3$beta-bb
