...
for(i in 1:5){
  ...
  for(ch in 4:6){
    ...
    l_diff_mat[[i]][[ct]]$kappa <- round( l_changed_graphs[[i]][[ch]]$kappa-l_changed_graphs[[i]]$truegraph$kappa, 3)
  }
...
array_to_latex(l_diff_mat[[3]][[2]]$beta)
...
for(t in seq_along(n_tp)){
  for(g in 1:5){
      l_raw[[paste0("graph",g)]][[t]] <- lapply(l_changed_graphs[[paste0("graph",g)]],
