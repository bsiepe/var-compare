...
for(i in 1:5){
  ...
  for(ch in 4:6){
    ...
    # l_diff_mat[[i]][[ct]]$pcor <- round(-1*stats::cov2cor(l_diff_mat[[i]][[ct]]$kappa),3)
  }
...
array_to_latex(l_diff_mat[[1]][[1]]$beta)
array_to_latex(l_diff_mat[[1]][[2]]$beta)
array_to_latex(l_diff_mat[[1]][[3]]$beta)
...
array_to_latex(l_diff_mat[[3]][[3]]$pcor)
...
for(t in seq_along(n_tp)){
      l_raw[[t]] <- list()
  for(g in 1:5){
      l_raw[[t]][[g]] <- lapply(l_changed_graphs[[g]],
...
for(t in seq_along(n_tp)){
  for(g in 1:5){
    l_raw[[g]][[t]]$permute <- lapply(l_raw[[g]][[t]]$truegraph,
                                  function(x) {
...
for(g in v_graphs){
  ...
  p <- df_comp_res %>% 
          ...
          # we have 1000 draws, so divide by 10
