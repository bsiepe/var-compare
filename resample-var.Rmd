...
bay_res2 <- var_estimate(dat2)
bay_res3 <- var_estimate(dat3)
```

## Directly use posterior samples

Take the first 100 samples from the posterior (after deleting warmup samples).
```{r}
dgp <- bay_res$fit$beta[,,51:151]
...
Refit both models to all 100 datasets. Use the internal function by Williams, modified by me.
...
l_refit2 <- foreach(i = 1:100) %dopar% {
  ref <- predict_pmu.var_estimate(object = bay_res2,
                 data = l_dat[[i]])

}

l_refit3 <- foreach(i = 1:100) %dopar% {
  ref <- predict_pmu.var_estimate(object = bay_res3,
                 data = l_dat[[i]])
...
We use RMSE to evaluate the fit, but could also use Bayesian $R^2$ or similar stuff (maybe even the likelihood). As we get credible intervals for the predictions, we could also do something with structure recovery, so see which edges were included and which weren't.

```{r}
source("aux_funs.R")
# Get Prediction errors across all datasets
df_rmse <- data.frame(iter = 1:length(l_dat),
                    rmse = rep(NA, length(l_dat)),
                    model = rep(1, length(l_dat)))
for(i in 1:length(l_dat)){
  df_rmse[i,"rmse"] <- f_eval_refit(data = l_dat[[i]], refit = l_refit[[i]])
}
```
Get the fit of the second model (TODO rewrite as one loop):
```{r}
# Get Prediction errors across all datasets
df_rmse2 <- data.frame(iter = 1:length(l_dat),
                    rmse = rep(NA, length(l_dat)),
                    model = rep(2, length(l_dat)))
for(i in 1:length(l_dat)){
  df_rmse2[i,"rmse"] <- f_eval_refit(data = l_dat[[i]], refit = l_refit2[[i]])
}
...
Third dataset:
```{r}
df_rmse3 <- data.frame(iter = 1:length(l_dat),
                    rmse = rep(NA, length(l_dat)),
                    model = rep(3, length(l_dat)))
for(i in 1:length(l_dat)){
  df_rmse3[i,"rmse"] <- f_eval_refit(data = l_dat[[i]], refit = l_refit3[[i]])
}
df_errors <- rbind(df_rmse, df_rmse2, df_rmse3)
```
...
```{r}
ggplot(df_errors, aes(x = rmse, group = as.factor(model),fill = as.factor(model), col = as.factor(model)))+
...
Third, for later, just obtain the likelihoods of each model directly and then compare these across samples. 
```{r}
