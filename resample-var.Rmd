...
# write_rds(df_comp_res, here::here("data/df_comp_res_2303.RDS"))
df_comp_res <- readRDS(here::here("data/df_comp_res_2303.RDS"))
```
...
for(g in v_graphs){
  ...
  p <- df_comp_res %>% 
          ...
          rename("Beta" = fp_beta05,
                 "Pcor" = fp_pcor05) %>% 
          pivot_longer(cols = c("Beta", "Pcor"), names_to = "res") %>% 
          ggplot(aes(x = as.numeric(tp), y = value, col = comp)) + 
          geom_line()+
          facet_grid(.~ res)+
          theme_minimal()+
