...
```{r}
df_errors %>% 
  mutate(reference = case_when(model == 1 ~ "Reference",
                               TRUE ~ "Comparison")) %>% 
  mutate(reference = as.factor(reference)) %>% 
  filter(model < 10) %>% 
  ggplot(aes(x = rmse, group = reference , fill = as.factor(model), col = as.factor(model)))+
...
Another idea: use a commonly understandable metric. As we have paired error statistics (two models on the same data), we can just compute the probability that a given model fits as good or better as the reference model. This does not say anything about the magnitude of the difference, which is why models that fit slightly worse might fare badly in this approach.
```{r}
