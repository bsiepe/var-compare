...
# Sanity check: Did this actually produce a chain graph?
...
n_postds <- 110  # number of posterior datasets
seed <- 2022
...
Comparison of results with grahpicalVAR is done in another script ("bggm-var-investigation.Rmd"). Here, we look at how well our model recovered the correct parameters. This function does not work anymore for the simulation structure. 
```{r recovery}
rec_res <- list()
...
Rewrite for all simulation conditions
```{r}
...
# TODO maxdiff still throws an error here
...
We want:
- Information about dgp
- Information about timepoint condition
- information about change of graph
- information about which model
...
ggsave("eval_plot.svg", device = "svg", path = here("figures/"))  
...
within_compare_eval <- function(l_res,
  ...
  if(isTRUE(pcor)){
    ### Pcor
    df_res_pcor <- subset(df_res, mat == "beta")
    # Obtain model indexes
