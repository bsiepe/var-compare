...
n_postds <- 150     # number of posterior datasets to create. I create more than I want to use in case some of the data generation fails
# probably only need 150 postds, not 200. Usually, only a couple of estimations fail
...
l_raw$graph <- sim_raw_parallel(dgp = graph, n = n_ind*1.5, 
                                tp = n_tp, means = 0,
                                standardize = TRUE)
l_raw$graph2 <- sim_raw_parallel(dgp = graph2, n = n_ind*1.5, 
                                 tp = n_tp, means = 0,
                                 standardize = TRUE)
...
# before <- Sys.time()
# 
# l_postrestest <- lapply(l_data, function(x) fit_var_parallel(data = x, n = 100,
#                  rho_prior = rho_sd, beta_prior = beta_sd, seed = seed,
...
l_postres_all <- fit_var_parallel_post(data = l_data, n = 100, nds = 200,
                 rho_prior = rho_sd, beta_prior = beta_sd, seed = seed,
                 iterations = n_iter, posteriorsamples = TRUE, pruneresults = TRUE) 
...
# testres<- lapply(testlist, function(x) fit_var_parallel2(data = x, n = 200,
#                  rho_prior = rho_sd, beta_prior = beta_sd, seed = seed,
#                  iterations = n_iter, posteriorsamples = TRUE, pruneresults = TRUE))
...
comp_conditions <- data.frame(mod_a = rep(comp_combinations[,1], 3),
                        ...
                        fitpost_a = rep("l_postres_fullcut", nrow(comp_combinations)*3),
                        fitpost_b = rep("l_postres_fullcut", nrow(comp_combinations)*3), 
                        fitemp_a = rep("l_res", nrow(comp_combinations)*3),
                        fitemp_b = rep("l_res", nrow(comp_combinations)*3),
                        n_datasets = rep(100, nrow(comp_combinations)*3),
...
plot_test_samedgp_eps500 <- df_eval_res %>% 
  ...
  labs(title = "Count of posterior differences larger than empirical",
       ...
       caption = "500 Timepoints, 100 Individuals")+
  facet_wrap(~comp)+
  theme(legend.position = "none")
ggsave("figures/plot_test_samedgp_eps500.svg", plot_test_samedgp_eps500,  device = "svg",
...
l_postres2_full <- fit_var_parallel_post(data = l_data2, n = 100, nds = 200,
                 rho_prior = rho_sd, beta_prior = beta_sd, seed = seed,
                 iterations = n_iter, posteriorsamples = TRUE, pruneresults = TRUE) 
...
# Cut away failed attempts, always get 100 fitted results
l_postres2_fullcut <- lapply(l_postres2_full, function(x) x[!sapply(x, is.null)])
...
comp_conditions2 <- data.frame(mod_a = rep(comp_combinations2[,1], 3),
                        ...
                        fitpost_a = rep("l_postres_fullcut", nrow(comp_combinations2)*3),
                        fitpost_b = rep("l_postres2_fullcut", nrow(comp_combinations2)*3), 
