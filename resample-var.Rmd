...
l_changed_graphs <- lapply(l_graphs, 
                           function(x) {change_graphs(
                                            ...
                                            changemax = c(1.4, 1.6),
                                            noise = c(0.1,0.2,0.3),
                                            permute_index = c(3,5,1,4,6,2),
                                            seed = seed)})

# write_rds(l_changed_graphs, "l_changed_graphs.RDS")
# write_rds(l_graphs, "l_graphs.RDS")
...
<!-- First, I add a new change condition: I swap columns in estimated models, e.g. swap variable 1 <-> variable 3 and variable 2 <-> variable 4.  -->
<!-- ```{r} -->
<!-- fit_test_new <- lapply(fit_test, function(x){ -->
<!--   x$beta_mu <- permute_mat_col(x$beta_mu) -->
<!--   x$pcor_mu <- permute_mat_col(x$pcor_mu, symmetric = TRUE) -->
<!--   return(x) -->
<!-- }) -->
<!-- ``` -->
...
parLapply(cl = cl, 1:nrow(comp_grid), function(n){
  ...
  wc_res  <- lapply(c(1:nrow(comp_ind)), function(x){
    ...
    wc_res[[x]] <- list()
    wc_res[[x]] <- within_compare(mod_a = ma,
                      mod_b = mb,
                      fitpost_a = l_ref,
                      fitpost_b = l_comp,
                      fitemp_a = l_ref,
...
df_comp_res <- readRDS("df_comp_res.RDS")
```
