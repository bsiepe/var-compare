...
l_raw <- list()
l_raw$graph4 <- l_raw$graph3 <- l_raw$graph2 <- l_raw$graph1 <-  list()
...
# write_rds(l_raw, "data/l_raw_new_0703.RDS")
...
dgp_names <- c("graph1", "graph2", "graph3", "graph4")
...
ncores <- 3
cl <- makeCluster(ncores)
clusterExport(cl, c("comp_grid",  "comp_grid_mod", "within_compare", "within_compare_eval", "post_distance_within"))
...
Read in results again, they were saved as individuals files. 
```{r}
...
df_comp_res %>% 
  filter(is.na(emp)) %>% 
  count()    # 103800/3145500 did not converge
# everything in the change1.6 condition
