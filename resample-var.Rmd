...
comp_conditions2 <- data.frame(mod_a = rep(comp_combinations[,1], 2),
...
df_eval_res <- as.data.frame(do.call(rbind, eval_res2))
df_eval_res %>% 
...
plot_test_comp <- df_eval_res %>% 
  ...
  labs(title = "Count of posterior differences larger than empirical",
       subtitle = "Simulating from different DGP (largeste edge *1.5)",
       caption = "100 Timepoints")+
  facet_wrap(~matrix)

ggsave("figures/plot_test_comp2.pdf", plot_test_comp2,  device = "pdf",
