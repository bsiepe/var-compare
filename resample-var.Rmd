...
set.seed(2022)
graph <- randomGVARmodel(Nvar = 6, probKappaEdge = 0.1, probBetaEdge = .85)
...
sum(df_ks_res$pval < 0.05, na.rm = TRUE)
...
# For now, just filter unsuccesful attempts
...
m1 <- l_res[[1]]$beta_mu+rnorm(36, sd = .35)

norm(l_res[[1]]$beta_mu-m1, type = "F")


# just change largest edge by 25%
ind_largeedge<- which.max(l_res[[1]]$beta_mu)
m2 <-l_res[[1]]$beta_mu
m2[ind_largeedge] <- m2[ind_largeedge]*1.75
...
# generates two new datasets, reestimates the model and then 
