...
# l_res <- list()
# l_res$graph4 <- l_res$graph3 <- l_res$graph2 <- l_res$graph1 <-  list()

# Only compute for permutation
l_raw <- lapply(l_raw, function(x) lapply(x, function(y) list(permute = y$permute)))
...
```{r}
# l_copy <- list.files(path = here::here("data/compare_sim_fits"), pattern = "graph2")
...
parLapply(cl = cl, todo_files_num, function(n){
  ...
  ## Read files
  # prevents reload of already loaded ref file
  # if(n > 1 & comp_grid$ref_file[n] == comp_grid$ref_file[n-1]){
...
todo_files_num <- desired_files %>% 
  filter(!filename %in% exist_files) %>% 
  filter(!grepl("graph1", filename)) %>% 
  filter(!grepl("graph2", filename)) %>% 
  pull(rownum)
...
# write_rds(df_comp_res, here::here("data/df_comp_res_2003.RDS"))
df_comp_res <- readRDS(here::here("data/df_comp_res_2003.RDS"))
```
...
Plot results. This is still too much code, I will collapse this later so that it iterates over the different DGPs. 
...
```{r}
plot_within_power05_graph1 <- df_comp_res %>% 
...
ggsave("plot_within_power05_graph1.svg", plot_within_power05_graph1, device = "svg", path = here("figures/"))  

plot_within_power05_graph2 <- df_comp_res %>% 
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>% 
  filter(dgp == "graph2") %>% 
  mutate(sig05 = ifelse(value < 50, 1, 0),
         sig01 = ifelse(value < 10, 1, 0)) %>%
...
ggsave("plot_within_power05_graph2.svg", plot_within_power05_graph2, device = "svg", path = here("figures/"))  

plot_within_power05_graph3 <- df_comp_res %>% 
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>% 
  filter(dgp == "graph3") %>% 
  mutate(sig05 = ifelse(value < 50, 1, 0),
         sig01 = ifelse(value < 10, 1, 0)) %>%
...
ggsave("plot_within_power05_graph3.svg", plot_within_power05_graph3, device = "svg", path = here("figures/"))  

plot_within_power05_graph4 <- df_comp_res %>%
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>%
  filter(dgp == "graph4") %>%
  mutate(sig05 = ifelse(value < 50, 1, 0),
         sig01 = ifelse(value < 10, 1, 0)) %>%
  ...
  scale_color_okabe_ito()
# 
ggsave("plot_within_power05_graph4.svg", plot_within_power05_graph4, device = "svg", path = here("figures/"))

plot_within_power05_graph5 <- df_comp_res %>%
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>%
  filter(dgp == "graph5") %>%
  mutate(sig05 = ifelse(value < 50, 1, 0),
         sig01 = ifelse(value < 10, 1, 0)) %>%
  ...
  facet_grid(change ~ res)+
  theme_minimal()
ggsave("plot_within_power05_graph5.svg", plot_within_power05_graph4, device = "svg", path = here("figures/"))
...
```{r}
plot_within_raw_graph1 <- df_comp_res %>% 
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>% 
  ...
  dplyr::summarize(pct_larger = mean(value/10)) %>%
  ggplot(aes(x = as.numeric(tp), y = pct_larger, col = comp)) + 
  # geom_text(aes(label = round(pct_larger,1)), size = 4)+
  geom_line()+
  facet_grid(change ~ res)+
  theme_minimal()+
  labs(caption = "% of post. diff. larger than empirical. Graph1.")
ggsave("plot_within_raw_graph1.svg", plot_within_raw_graph1, device = "svg", path = here("figures/"))  


plot_within_raw_graph2 <- df_comp_res %>% 
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>% 
  # mutate(across(c(comp, dgp, tp, comp_graph, res),
  # ~ as.factor(.))) %>% 
  filter(dgp == "graph2") %>% 
  ...
  theme_minimal()+
  scale_color_okabe_ito()+ 
  labs(caption = "% of post. diff. larger than empirical. graph2.")
ggsave("plot_within_raw_graph2.svg", plot_within_raw_graph2, device = "svg", path = here("figures/"))  



plot_within_raw_graph3 <- df_comp_res %>% 
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>% 
  # mutate(across(c(comp, dgp, tp, comp_graph, res),
  # ~ as.factor(.))) %>% 
  filter(dgp == "graph3") %>% 
  ...
  theme_minimal()+
  scale_color_okabe_ito()+ 
  labs(caption = "% of post. diff. larger than empirical. graph3.")
ggsave("plot_within_raw_graph3.svg", plot_within_raw_graph3, device = "svg", path = here("figures/"))

plot_within_raw_graph4 <- df_comp_res %>% 
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>% 
  # mutate(across(c(comp, dgp, tp, comp_graph, res),
  # ~ as.factor(.))) %>% 
  filter(dgp == "graph4") %>% 
  ...
  theme_minimal()+
  scale_color_okabe_ito()+ 
  labs(caption = "% of post. diff. larger than empirical. graph5.")
ggsave("plot_within_raw_graph4.svg", plot_within_raw_graph4, device = "svg", path = here("figures/")) 

plot_within_raw_graph5 <- df_comp_res %>% 
  pivot_longer(cols = c("beta_a", "beta_b", "pcor_a", "pcor_b"), names_to = "res") %>% 
  # mutate(across(c(comp, dgp, tp, comp_graph, res),
  # ~ as.factor(.))) %>% 
  filter(dgp == "graph5") %>% 
  ...
  scale_color_okabe_ito()+ 
  labs(caption = "% of post. diff. larger than empirical. graph5.")
ggsave("plot_within_raw_graph5.svg", plot_within_raw_graph5, device = "svg", path = here("figures/")) 

```

#### False Positives
Plot false positives across graphs. 
This does not account for the correct dgp at the moment. 
```{r}
plot_within_fp05_graph1 <- df_comp_res %>% 
  # here, beta_b and pcor_b are irrelevant, as we compare within the same graph
  pivot_longer(cols = c("beta_a", "pcor_a"), names_to = "res") %>% 
  filter(dgp == "graph1") %>%
  filter(change == "truegraph") %>% 
  mutate(sig05 = ifelse(value < 50, 1, 0),
         sig01 = ifelse(value < 10, 1, 0)) %>%
  group_by(dgp, comp, tp, change, res) %>%
  mutate(n_comp = n()) %>%  
  summarize(fp05 = sum(sig05)/n_comp) %>% 
  # somehow does not cut everything if I don't do this
  summarize(fp05 = mean(fp05)) %>% 
  ggplot(aes(x = as.numeric(tp), y = fp05, col = comp)) + 
  geom_line()+
  facet_grid(dgp ~ res)+
  theme_minimal()+
  scale_color_okabe_ito()+
  ylim(c(-0.01, 0.2))+
  labs(caption = "Percentage of false positives for different norms.")

ggsave("plot_within_fp05_graph1.svg", plot_within_fp05_graph1, device = "svg", path = here("figures/"))  

plot_within_fp05_graph2 <- df_comp_res %>% 
  # here, beta_b and pcor_b are irrelevant, as we compare within the same graph
  pivot_longer(cols = c("beta_a", "pcor_a"), names_to = "res") %>% 
  filter(change == "truegraph") %>% 
  filter(dgp == "graph2") %>% 
...
ggsave("plot_within_fp05_graph2.svg", plot_within_fp05_graph2, device = "svg", path = here("figures/")) 



plot_within_fp05_graph3 <- df_comp_res %>% 
  # here, beta_b and pcor_b are irrelevant, as we compare within the same graph
  pivot_longer(cols = c("beta_a", "pcor_a"), names_to = "res") %>% 
  filter(change == "truegraph") %>% 
  filter(dgp == "graph3") %>% 
...
ggsave("plot_within_fp05_graph3.svg", plot_within_fp05_graph3, device = "svg", path = here("figures/")) 


plot_within_fp05_graph4 <- df_comp_res %>% 
  # here, beta_b and pcor_b are irrelevant, as we compare within the same graph
  pivot_longer(cols = c("beta_a", "pcor_a"), names_to = "res") %>% 
  filter(change == "truegraph") %>% 
  filter(dgp == "graph4") %>% 
...
ggsave("plot_within_fp05_graph4.svg", plot_within_fp05_graph4, device = "svg", path = here("figures/")) 

plot_within_fp05_graph5 <- df_comp_res %>% 
  # here, beta_b and pcor_b are irrelevant, as we compare within the same graph
  pivot_longer(cols = c("beta_a", "pcor_a"), names_to = "res") %>% 
  filter(change == "truegraph") %>% 
  filter(dgp == "graph5") %>% 
...
ggsave("plot_within_fp05_graph5.svg", plot_within_fp05_graph5, device = "svg", path = here("figures/")) 
...
```{r}
plot_dec_rule_graph1 <- df_comp_res %>% 
  filter(dgp == "graph1") %>% 
  # setup decision rule
  ...
  theme_minimal()+
  scale_color_okabe_ito()+
  labs(caption = "Power of decision rule \"OR\" for graph1")
ggsave("plot_dec_rule_graph1.svg", plot_dec_rule_graph1, device = "svg", path = here("figures/")) 


plot_dec_rule_graph2 <- df_comp_res %>% 
  # dplyr::select(-c("kappa_a", "kappa_b")) %>% 
  filter(dgp == "graph2") %>% 
  # setup decision rule
  mutate(sig_beta05 = ifelse(beta_a < 50 | beta_b < 50, 1, 0),
         sig_pcor05 = ifelse(pcor_a < 50 | pcor_b < 50, 1, 0)) %>% 
  ...
  theme_minimal()+
  scale_color_okabe_ito()+
  labs(caption = "Power of decision rule \"OR\" for graph2")
ggsave("plot_dec_rule_graph2.svg", plot_dec_rule_graph2, device = "svg", path = here("figures/")) 


plot_dec_rule_graph3 <- df_comp_res %>% 
  # dplyr::select(-c("kappa_a", "kappa_b")) %>% 
  filter(dgp == "graph3") %>% 
  # setup decision rule
  mutate(sig_beta05 = ifelse(beta_a < 50 | beta_b < 50, 1, 0),
  ...
  theme_minimal()+
  scale_color_okabe_ito()+
  labs(caption = "Power of decision rule \"OR\" for graph3")
ggsave("plot_dec_rule_graph3.svg", plot_dec_rule_graph3, device = "svg", path = here("figures/")) 


plot_dec_rule_graph4 <- df_comp_res %>% 
  filter(dgp == "graph4") %>% 
  # setup decision rule
  mutate(sig_beta05 = ifelse(beta_a < 50 | beta_b < 50, 1, 0),
         sig_pcor05 = ifelse(pcor_a < 50 | pcor_b < 50, 1, 0)) %>% 
  ...
  theme_minimal()+
  # scale_color_okabe_ito()+
  labs(caption = "Power of decision rule \"OR\" for graph4")
ggsave("plot_dec_rule_graph4.svg", plot_dec_rule_graph4, device = "svg", path = here("figures/")) 


plot_dec_rule_graph5 <- df_comp_res %>% 
  filter(dgp == "graph5") %>% 
  # setup decision rule
  mutate(sig_beta05 = ifelse(beta_a < 50 | beta_b < 50, 1, 0),
         sig_pcor05 = ifelse(pcor_a < 50 | pcor_b < 50, 1, 0)) %>% 
  ...
  theme_minimal()+
  # scale_color_okabe_ito()+
  labs(caption = "Power of decision rule \"OR\" for graph5")
ggsave("plot_dec_rule_graph5.svg", plot_dec_rule_graph5, device = "svg", path = here("figures/")) 
```
Plot decision rule for all graphs, seperately for beta and pcor
```{r}
plot_dec_rule_beta <- df_comp_res %>% 
  # setup decision rule
  ...
  dplyr::summarize(power_beta05 = sum(sig_beta05)/n_comp,
            power_pcor05 = sum(sig_pcor05)/n_comp) %>% 
  # somehow does not cut everything if I don't do this because of n_comp
  dplyr::summarize(power_beta05 = mean(power_beta05),
            power_pcor05 = mean(power_pcor05)) %>% 
  dplyr::select(-contains("pcor")) %>% 
  # pivot_longer(cols = c("power_beta05", "power_pcor05"), names_to = "res") %>% 
  ggplot(aes(x = as.numeric(tp), y = power_beta05, col = comp)) + 
  geom_line()+
  facet_grid(change ~ dgp)+
  theme_minimal()+
  scale_color_okabe_ito()+
  labs(caption = "Power of decision rule \"OR\" for beta")
ggsave("plot_dec_rule_beta.svg", plot_dec_rule_beta, device = "svg", path = here("figures/")) 
```
Same for Pcor
```{r}
plot_dec_rule_pcor <- df_comp_res %>% 
  # setup decision rule
  dplyr::mutate(sig_beta05 = ifelse(beta_a < 50 | beta_b < 50, 1, 0),
         sig_pcor05 = ifelse(pcor_a < 50 | pcor_b < 50, 1, 0)) %>% 
  dplyr::group_by(dgp, comp, tp, change) %>% 
  ...
  # pivot_longer(cols = c("power_beta05", "power_pcor05"), names_to = "res") %>% 
  ggplot(aes(x = as.numeric(tp), y = power_pcor05, col = comp)) + 
  geom_line()+
  facet_grid(change ~ dgp)+
  theme_minimal()+
  scale_color_okabe_ito()+
  labs(caption = "Power of decision rule \"OR\" for pcor")
ggsave("plot_dec_rule_pcor.svg", plot_dec_rule_pcor, device = "svg", path = here("figures/")) 
```
...
```{r}
plot_emp_norms_graph1 <- df_comp_res %>% 
  # dplyr::select(-c("kappa_a", "kappa_b")) %>% 
  # mutate(across(c(comp, dgp, tp, comp_graph, res),
  # ~ as.factor(.))) %>% 
  filter(dgp == "graph1") %>% 
  ...
  labs(caption = "Empirical distance for different norms with SDs.",
       x = "tp",
       y = "Mean Distance")
ggsave("plot_emp_norms_graph1.svg", plot_emp_norms_graph1, device = "svg", path = here("figures/"))  



plot_emp_norms_graph2 <- df_comp_res %>% 
  dplyr::select(-c("kappa_a", "kappa_b")) %>% 
  # mutate(across(c(comp, dgp, tp, comp_graph, res),
  # ~ as.factor(.))) %>% 
  filter(dgp == "graph2") %>% 
  ...
  labs(caption = "Empirical distance for different norms with SDs.",
       x = "tp",
       y = "Mean Distance")
ggsave("plot_emp_norms_graph2.svg", plot_emp_norms_graph2, device = "svg", path = here("figures/"))  



plot_emp_norms_graph3 <- df_comp_res %>% 
  dplyr::select(-c("kappa_a", "kappa_b")) %>% 
  # mutate(across(c(comp, dgp, tp, comp_graph, res),
  # ~ as.factor(.))) %>% 
  filter(dgp == "graph3") %>% 
  ...
  labs(caption = "Empirical distance for different norms with SDs.",
       x = "tp",
       y = "Mean Distance")
ggsave("plot_emp_norms_graph3.svg", plot_emp_norms_graph3, device = "svg", path = here("figures/"))  
