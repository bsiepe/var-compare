...
```{r change-graphs}
l_graphs <- list(graph1 = list(),
                 graph2 = list(),
                 graph3 = list())

# TODO 
# need to decide on which noise we add
# Write two functions that change values of each graph 
# and double check if it worked
# they need to store the information about the change somewhere
change_graphs <- function(truegraph, 
                          changemax, 
                          noise){
  ### Obtain matrix characteristics
  ## Beta
  b_i <- nrow(truegraph$beta)
  b_j <- ncol(truegraph$beta)
  ## Kappa
  k_i <- nrow(truegraph$kappa)
  k_j <- ncol(truegraph$beta)
  
  ### Find max
  ## Beta
  b_max <- which.max(truegraph$beta)
  ...
  max_row <- NA
  max_col <- NA
  # TODO change m_kappa to something different
  ...
  m_kappa[max_row,max_col] <- m_kappa[max_row,max_col]*change
  m_kappa[max_col,max_row] <- m_kappa[mxa_col,max_row]*change
  
  
  ### Add noise
  ## Beta
  truegraph$beta + runif(n = b_i*b_j, min = noise$min, max = noise$max)
  
  ## Kappa
  kappa_tmp <- truegraph$kappa + runif(n = k_i*k_j, min = noise$min, max = noise$max)
  kappa_tmp <- as.matrix(Matrix::forceSymmetric(kappa_tmp))
...
n_postds <- 150     # number of posterior datasets
...
l_raw$graph <- sim_raw_parallel(dgp = graph, n = n_postds, 
                                tp = n_tp, means = 0,
                                standardize = TRUE)
l_raw$graph2 <- sim_raw_parallel(dgp = graph2, n = n_postds, 
                                 tp = n_tp, means = 0,
                                 standardize = TRUE)
stopCluster(cl)
```
...
Compute Frobenius norm between models on resampled datasets compared to reference model and then compare to actual difference between two models. 
...
comp_conditions <- data.frame(
                        postpost = rep(FALSE, nrow(comp_combinations)*3),
                        mod_a = rep(comp_combinations[,1], 3),
