...
# saveRDS(df_norms, file = here("data/revision1/norm_distributions/df_norms.RDS"))
df_norms <- readRDS(here("data/revision1/norm_distributions/df_norms.RDS"))
```
...
df_norms %>% 
  ...
  separate(stat, into = c("mat", "stat"), sep = c("\\.")) %>% 
  separate(mat, into = c("mat", "type"), sep = c("_")) %>% 
  group_by(dgp, change, tp, comp, mat, type) %>% 
  ...
  gt_theme_538() %>% 
  gtsave(here::here("figures/revision1/empirical_norms_mean.html"))
...
df_norms %>% 
  ...
  mutate(diff = ifelse(post > ref, 1, 0)) %>% 
  ...
  gtsave(here::here("figures/revision1/empirical_norms_mean_diff_detect.html"))
...
l_comp_res <- list()
l_comp_res <- foreach(i = 1:length(file_list)) %dopar% {
  comp_res <- readRDS(here::here(paste0("data/compare_sim_res_revision/",file_list[i])))
  ...
  eval_res <- lapply(comp_res, function(x){
    ...
    # TODO add new testing function here
    # res <- tryCatch({within_compare_eval(x, kappa = FALSE)}, error = function(e) NA)
    res <- tryCatch({test_between(x)}, error = function(e) NA)
    res$dgp <- dgp_it
...
plot_dec_rule_se
ggsave(filename = "plot_dec_rule_se.svg",plot_dec_rule_se, device = "svg", path = here::here("figures/"), width = 14, height = 12)
