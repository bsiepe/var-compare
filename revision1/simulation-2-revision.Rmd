...
# because some models don't converged previously
# now there is no longer any nonconvergence
...
model_combs <- generate_unique_pairs(n = 100, count = 1000)
sort_and_concat <- function(row) {
  toString(sort(row))
}

# Applying the function to each row of the data frame
sorted_df <- apply(model_combs, 1, sort_and_concat)

# Checking for duplicate rows in the sorted data frame
duplicate_rows <- duplicated(sorted_df) | duplicated(sorted_df, fromLast = TRUE)
sum(duplicate_rows)
...
cl = makeCluster(ncores)
registerDoParallel(cl)
clusterExport(cl, c("file_list", "within_compare_eval", "post_distance_within", "here"))
l_comp_res <- foreach(i = 1:length(file_list)) %dopar% {
  ...
  eval_res <- lapply(comp_res, function(x){
    ...
    res <- tryCatch({within_compare_eval(x)}, error = function(e) NA)
    res$dgp <- dgp_it
    res$change <- change_it
    res$tp <- tp_it
    res$comp <- comp_it
  ...
  # Save output
  saveRDS(df_eval_res, file = here::here(paste0("data/revision1/compare_sim_eval/eval_res_",dgp_it, "_",change_it,"_", tp_it, "_",comp_it, ".RDS")))
...
```{r}
# For wide priors
file_list <- list.files(path = here("data/revision1/compare_sim_eval"), full.names = TRUE)
file_list_w <- file_list[-grep("_narrow_", file_list)]
l_comp_res_w <- lapply(file_list_w, readRDS)
df_comp_res_w <- do.call(rbind, l_comp_res_w)

# write_rds(df_comp_res_w, here::here("data/revision1/df_comp_res_wide_rev1.RDS"))
# df_comp_res_w <- readRDS(here::here("data/revision1/df_comp_res_wide_rev1.RDS"))
...
# write_rds(df_comp_res_n, here::here("data/revision1/df_comp_res_narrow_rev1.RDS"))
# df_comp_res_n <- readRDS(here::here("data/revision1/df_comp_res_narrow_rev1.RDS"))
...
write_rds(df_comp_res, here::here("output/revision1/df_comp_res_rev1.RDS"))
df_comp_res <- readRDS(here::here("output/revision1/df_comp_res_rev1.RDS"))
...
Dataset explanation:
- df_comp_res_rev1 is with the OR rule
- df_comp_res_rev1_old_idea is with the COMB rule
...
plot_test_null_dist <- df_comp_res %>% 
          ...
          geom_histogram(bins = 30, fill = ggokabeito::palette_okabe_ito()[2])+
...
plot_test_null_dist
ggsave(filename = "plot_test_null_dist_wide_rev1.svg",plot_test_null_dist, device = "svg",
...
plot_dec_rule_se <- df_comp_res %>%
  ...
  dplyr::summarize(
    ...
    se_beta05 = sqrt((power_beta05 * (1 - power_beta05)) /
                       n_comp),
    se_pcor05 = sqrt((power_pcor05 * (1 - power_pcor05)) /
                       n_comp)
  ) %>%
  tidyr::pivot_longer(cols = c("power_beta05", "power_pcor05"),
                      names_to = "res") %>%
  # attach correct se
  ...
  scale_y_continuous(limits = c(0, 1), 
                     ...
                     labels = c(0, "", 0.5, "", 1)) +
  scale_x_discrete()+
  geom_vline(colour = "#F3F4F5",
             xintercept = seq(1.5, length(tp_levels), 1)) +
  theme(
    ...
    legend.text = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.5)),
    legend.title = ggplot2::element_text(face = "bold", size = ggplot2::rel(1.5)),
    panel.spacing.y = ggplot2::unit(1.6, "lines"),
    panel.spacing.x = ggplot2::unit(1.6, "lines"),
    axis.line = element_line(colour = "#6d6d6e"),
    ...
    axis.text.y = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.2)),
    axis.title.x = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.5)),
    axis.title.y = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.5)),
    panel.grid.major.x = element_blank()
  )

plot_dec_rule_se
ggsave(filename = "plot_dec_rule_se_rev2.svg",plot_dec_rule_se, device = "svg", path = here::here("figures/revision2"), width = 14, height = 12)
...
ggsave(filename = paste0("plot_false_positive_se_rev2.svg"),plot_false_positive_se, device = "svg",
         path = here::here("figures/revision2/"), width = 13, height = 6)
```
...
ggsave(filename ="plot_dec_rule_prior_sens_rev2.svg",plot_dec_rule_prior_sens, device = "svg",
         path = here::here("figures/revision2/"), width = 13, height = 10)
```
...
ggsave(filename = paste0("plot_false_positive_se_prior_sens_rev2.svg"),plot_false_positive_se_prior_sens, device = "svg",
         path = here::here("figures/revision2/"), width = 13, height = 6)
