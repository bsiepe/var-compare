...
# Load DGPs
# They are created in true-networks.Rmd
...
ls_eval_bggm_both_plots[[1]] <- ggplot(data = ls_data_eval_bggm$Bias, 
  ...
  geom_errorbar(aes(ymin = value - 1*mcse,
                            ymax = value + 1*mcse),
                        width = .9, 
...
ggsave("plot_eval_bggm_rev1.svg", plot_eval_bggm_separate, height = 9, width = 12, path = here::here("figures/revision1/"), device = "svg")
...
ggsave("plot_eval_graphicalvar_rev1.svg", plot_eval_graphicalvar, height = 9, width = 12, path = here::here("figures/revision1/"), device = "svg")
```
...
ggsave("plot_eval_both_cor_separate_rev1.svg", plot_eval_both_separate, height = 9, width = 12, path = here::here("figures/revision1/"), device = "svg")



```

Plot only bias for the revision: 
```{r}
plot_bias <- ggplot(data = ls_data_both_stat$Bias, 
         aes(x = tp_ind, y = value, col = method, linetype = method, group = method))+
  geom_errorbar(aes(ymin = value - 1*mcse,
                            ymax = value + 1*mcse),
                        width = .9, 
  ...
  theme(strip.text = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.3), 
        ...
        strip.text.x.top = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.3),
                                                 hjust = 0.5),
        # strip.text.y = element_blank(),
        legend.text = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.3)),
        legend.title = ggplot2::element_text(face = "bold", size = ggplot2::rel(1.3)),
        axis.line = element_line(colour = "#6d6d6e"),
        panel.spacing.y = ggplot2::unit(1.6, "lines"),
        panel.spacing.x = ggplot2::unit(1.6, "lines"),
        axis.text.x = ggplot2::element_text(face = "plain", size = ggplot2::rel(1.2)),
...
ggsave("plot_eval_both_bias_rev2.svg", plot_bias, 
       height = 8, width = 11, path = here::here("figures/revision2"), device = "svg")
...
plot_eval_select
ggsave("plot_eval_select_rev1.svg", plot_eval_select, height = 9, width = 12, path = here::here("figures/revision1/"), device = "svg")


```


### Combine bias/cor and sensitivity/specificity in one

```{r}
# in 2 rows
patchwork::wrap_plots(plot_eval_both_separate + theme(legend.position = "none"), 
                      plot_eval_select, 
                      nrow = 2) + 
  patchwork::plot_layout(guides = "collect")


pes_grid <- plot_eval_select + theme(legend.position = "none")
...
plot_ci_sens <- df_eval_bggm %>% 
  ...
  geom_errorbar(aes(ymin = mean_stat - 1*mean_mcse,
                            ymax = mean_stat + 1*mean_mcse),
                        width = .8, 
                 position = position_dodge(0.7),
                 show.legend = FALSE)+
  # geom_line()+
...
ggsave("plot_ci_sens_rev1.svg", plot_ci_sens, height = 9, width = 12, path = here::here("figures/revision1/"), device = "svg")
...
ggsave("plot_eval_both_cor_nonselect_rev1.svg", plot_eval_both_nonselect, height = 9, width = 12, path = here::here("figures/revision1/"), device = "svg")
...
```{r}
plot_eval_credible <- df_eval_bggm %>% 
  # delete older dgps
  filter(dgp_ind > dgp_cut) %>%
  ...
  group_by(dgp_ind, tp_ind, prior_comb, stat, mat, intercept) %>%
  dplyr::summarize(value = mean(value, na.rm = TRUE)) %>%
  # add new mcse column which contains standard error of proportion from "value"
  mutate(mcse = sqrt(value *(1-value)/n_rep)) %>%
  ...
  geom_errorbar(aes(ymin = value - 1*mcse,
                 ...
                 show.legend = FALSE)+
  geom_point(size = 1.2, position = position_dodge(0.7))+
  scale_y_continuous(limits = c(.80,1))+
  ggh4x::facet_nested(as.factor(dgp_ind) ~ as.factor(stat) + as.factor(mat),
                      axes = TRUE,
                      remove_labels = TRUE)+
  scale_color_manual(values = cond_colors)+
...
plot_eval_credible 
ggsave("plot_eval_credible_rev1.svg", plot_eval_credible, height = 7, width = 14, path = here::here("figures/revision1/"), device = "svg")
