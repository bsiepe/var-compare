...
# Test after subtractring a reference distribution

Idea: Subtract the reference distribution from the posterior distance distribution, thereby accounting for the estimation uncertainty.
...
cgb13 <- compare_gvar_between(r_net_1a, r_net_3,
                     comp = "l1", 
                     n_draws = 10000,
                     combine_post = TRUE,
                     sampling_method = "random",
...
Plot the difference of disttributions
```{r}
plot_diff <- function(cgb_obj){
  cgb_obj %>% 
    mutate(beta = beta - beta_ref,
           pcor = pcor - pcor_ref) %>% 
    pivot_longer(c(beta, pcor), 
                 names_to = "mat", 
                 values_to = "diff") %>% 
    ggplot(aes(x = diff, fill = mat)) +
    geom_density() + 
    theme_compare() + 
